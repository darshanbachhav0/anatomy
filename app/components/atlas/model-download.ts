...
export async function decodeModelResponse(response: Response, expectedBytes: number, compressed: boolean): Promise<ArrayBuffer> {
    if (!response.ok)
        throw new Error('No se pudo descargar un archivo de anatomía. Comprueba la conexión y reintenta.');
    ...
    if (buffer.byteLength !== expectedBytes)
        throw new Error('Un archivo de anatomía está incompleto. Reintenta la carga del visor.');
    return buffer;
}
